...
class LowThrustTransfer(pk.trajopt._direct._direct_base):
    ...
    def __init__(self,
        ...
        # initialise base
        super().__init__(mass, thrust, isp, nseg, pk.MU_SUN, hf)

        # planets
        self.p0 = pk.planet.jpl_lp(p0) if isinstance(p0, str) else p0
        self.pf = pk.planet.jpl_lp(pf) if isinstance(pf, str) else pf
    ...
    def get_bounds(self):
        lb = [self.t0[0], self.tof[0], self.sc.mass * 0.01] + [-self.vinf_dep] * 3 + [-self.vinf_arr] * 3 + [-1, -1, -1] * self.nseg
        ub = [self.t0[1], self.tof[1], self.sc.mass] + [self.vinf_dep] * 3 + [self.vinf_arr] * 3 + [1, 1, 1] * self.nseg
        return (lb, ub)
