...
def optimal_step_size(last_step, mean_error_ratio, safety=0.9, ifactor=10.0, dfactor=0.2, order=5.0):
    ...
    factor = jnp.maximum(1.0 / ifactor, jnp.minimum(err_ratio**(1.0 / order) / safety, 1.0 / dfactor))
...
def _odeint_wrapper(func, rtol: float, atol: float, mxstep: int, y0, ts, *args):
...
def _odeint(func, rtol: float, atol: float, mxstep: int, y0, ts, *args):
    func_ = lambda y, t: func(y, t, *args)

    # Outer loop: iterate over the desired time values and estimate the integral at each one
    def scan_fun(carry, target_t):
        # 'Middle' loop: take Runge-Kutta steps with different step sizes until the
        # estimated error is below the tolerance level
        def cond_fun(state):
